...
import { Role, Notificacion, NotificationType } from '../types.ts';
import TenantSelector from './TenantSelector.tsx';
import { LogOutIcon, UserCircleIcon, BellIcon, FacturasIcon, InventarioIcon, ConfiguracionIcon, ClipboardDocumentCheckIcon, SearchIcon } from './icons/Icons.tsx';
import { useUIStore } from '../stores/useUIStore.ts';
import ProfileModal from '../features/auth/ProfileModal.tsx';
...
const Header: React.FC = () => {
  ...
  const { notifications, checkSystemAlerts } = useNotificationStore();
    ...
    <>
      <header className="flex-shrink-0 flex items-center justify-between px-4 py-3 bg-white border-b border-secondary-200">
          ...
          {!isContador && selectedTenant && (
           ...
           <div className="hidden md:flex items-center ml-6">
            <button 
              ...
              className="flex items-center w-64 text-left p-2 bg-secondary-100 rounded-md hover:bg-secondary-200 transition-colors duration-200"
            >
              <SearchIcon className="h-5 w-5 text-secondary-500 mr-3" />
              <span className="text-sm text-secondary-500">Buscar...</span>
              <span className="ml-auto text-xs border bg-white rounded px-1.5 py-0.5">⌘K</span>
            </button>
          </div>
        </div>
        <div className="flex items-center space-x-2 md:space-x-4">
          ...
          <button onClick={() => setIsProfileModalOpen(true)} className="hidden md:flex items-center space-x-2 p-1 rounded-md hover:bg-secondary-100">
              {user?.photoURL ? (
